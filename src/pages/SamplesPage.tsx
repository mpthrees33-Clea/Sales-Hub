...
function NewOrderForm({ onSubmit }: { onSubmit: () => void }) {
  ...
  return (
    <div className="bg-white rounded-lg border border-slate-200 p-4 space-y-4">
      <h3 className="font-semibold text-slate-700 text-sm">New Sample Order</h3>

      {/* Step 1 — Customer */}
      <div>
        <label className="block text-xs font-medium text-slate-500 mb-1">1. Customer</label>
        ...
        {customer && customer.shipToAddresses.length > 1 && (
          <select className="w-full mt-1 text-xs border border-slate-200 rounded-lg px-3 py-2"
            onChange={(e) => {
      ...
      {/* Step 2 — Project */}
      <div>
        <label className="block text-xs font-medium text-slate-500 mb-1">2. Project</label>
        <select className="w-full text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      {/* Step 3 — Products */}
      <div>
        <label className="block text-xs font-medium text-slate-500 mb-1">3. Products</label>
        ...
        {searchResults.length > 0 && (
          <div className="border border-slate-200 rounded-lg mt-1 divide-y divide-slate-100 max-h-40 overflow-y-auto">
            {searchResults.map((p) => (
              <button key={p.id} type="button" onClick={() => addItem(p.id)}
                className="w-full text-left px-3 py-2 text-xs hover:bg-blue-50">
                <span className="font-medium text-slate-700">{p.trinityName}</span>
                <span className="text-slate-400 ml-2">({p.privateLabels.map((pl) => `${pl.brand} ${pl.productName}`).join(', ')})</span>
              </button>
            ))}
          </div>
        )}
        {items.length > 0 && (
          <div className="mt-2 space-y-1">
            {items.map((item) => (
              <div key={item.productId} className="flex items-center gap-2 text-xs bg-slate-50 rounded px-2 py-1.5">
                <span className="flex-1 font-medium text-slate-700">{item.productName}</span>
                <button onClick={() => updateQty(item.productId, -1)} className="p-0.5 text-slate-400 hover:text-slate-600"><Minus size={12} /></button>
                <span className="w-5 text-center font-medium">{item.quantity}</span>
                <button onClick={() => updateQty(item.productId, 1)} className="p-0.5 text-slate-400 hover:text-slate-600"><Plus size={12} /></button>
                <button onClick={() => removeItem(item.productId)} className="p-0.5 text-red-400 hover:text-red-600"><XIcon size={12} /></button>
              </div>
            ))}
          </div>
        )}
      ...
      <div className="space-y-2">
        <label className="block text-xs font-medium text-slate-500">4. Shipping</label>
        ...
        <div className="grid grid-cols-3 gap-2">
          <input className="col-span-1 text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
            placeholder="City" value={shipCity} onChange={(e) => setShipCity(e.target.value)} />
          <input className="text-sm border border-slate-200 rounded-lg px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
      ...
      <button onClick={submit}
        ...
        className="w-full py-2 bg-blue-600 text-white text-sm font-medium rounded-lg hover:bg-blue-700 disabled:opacity-40 disabled:cursor-not-allowed">
        Submit Order
      </button>
    </div>
  );
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
      ...
      <div className="lg:col-span-2">
        {success && (
          <div className="mb-3 px-3 py-2 bg-green-100 text-green-700 text-sm rounded-lg font-medium">
            Order submitted!
          </div>
        )}
      ...
      <div className="lg:col-span-3 space-y-3">
        <div className="flex gap-2 flex-wrap">
          {(['All','Pending','Processing','Shipped','Delivered'] as const).map((s) => (
            <button key={s} onClick={() => setFilter(s)}
              className={`px-3 py-1 text-xs font-medium rounded-full border transition-colors ${filter === s ? 'bg-blue-600 text-white border-blue-600' : 'bg-white text-slate-500 border-slate-200 hover:border-slate-300'}`}>
              {s}
            </button>
          ))}
        </div>
        ...
        <div className="space-y-2">
          {filtered.map((order) => {
            ...
            return (
              <div key={order.id} className="bg-white rounded-lg border border-slate-200 overflow-hidden">
                <button className="w-full text-left px-4 py-3 flex items-center gap-3 hover:bg-slate-50"
                  onClick={() => setExpanded(isExpanded ? null : order.id)}>
                  <ChevronDown size={14} className={`text-slate-400 transition-transform ${isExpanded ? '' : '-rotate-90'}`} />
                  <div className="flex-1 min-w-0">
                    <p className="font-medium text-sm text-slate-800">{customer?.company ?? '—'}</p>
                    <p className="text-xs text-slate-400">{order.orderedDate} · {order.items.length} item(s)</p>
                  </div>
                    ...
                    <div className="mt-2 space-y-1">
                      {order.items.map((item, i) => (
                        <div key={i} className="flex gap-2 text-xs text-slate-600">
                          <span className="w-5 h-5 bg-slate-200 rounded text-center leading-5 font-medium shrink-0">{item.quantity}</span>
                          <span>{item.productName}</span>
                          {item.privateLabelName && <span className="text-slate-400">({item.privateLabelName})</span>}
                        </div>
                      ))}
                    </div>
                    <p className="text-xs text-slate-500">{order.shippingName} · {order.shippingAddress}, {order.shippingCity}, {order.shippingState} {order.shippingZip}</p>
                    {order.trackingNumber && <p className="text-xs text-blue-600">Track: {order.trackingNumber}</p>}
          ...
          {filtered.length === 0 && <p className="text-sm text-slate-400 text-center py-8">No orders.</p>}
        </div>
      </div>
    </div>
  );
